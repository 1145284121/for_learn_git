...
uint32 i2c_init(I2Cn_e i2cn, uint32 baud)
{
    ...
    mult = SysClock.bus_khz*1000/baud/3840;

    scldiv =  SysClock.bus_khz * 1000 / ( (1<<mult) * baud );    //最佳的分频系数
    
    n = 0x40;
    while(n)                                                //循环里逐个扫描，找出最接近的 分频系数
    {
        n--;
        Dvalue = (scldiv - ICR_2_SCL_divider[n]) < 0 ? -(scldiv - ICR_2_SCL_divider[n]) : (scldiv - ICR_2_SCL_divider[n]);
        if(Dvalue == 0)
        {
            icr = n;
            break;                                          //退出while循环
    ...
    return (  SysClock.bus_khz * 1000 / ( (1<<mult) * ICR_2_SCL_divider[icr])  );
}
...
void i2c_delay(void)
{
    volatile uint16 n = 120;     //注意，这个数据太小，会导致读取错误。
