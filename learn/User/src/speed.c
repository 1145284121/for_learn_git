...
void PWMOut(void)
{
	...
	if(Flag_Speed == OFF)		
          g_fSpeedControlOut = 0;		//如果Flag_Speed == OFF		不输出速度
	if(Flag_Direction == OFF)	
          g_fDirectionControlOut = 0;	//如果Flag_Direction == OFF 不输出方向
...
void SpeedControl(void)
{
	...
	//flash中参数
	g_speedControl_P = 320;	//320
	g_speedControl_I = 100;	//100
	...
	g_fSpeedControlOut = (g_fSpeedControlOut>=6000?6000:g_fSpeedControlOut);        //输出限幅
